...
def run(input_path, output_path, model_path, model_type="dpt_hybrid", optimize=True):
    ...
    fPath = 'output_monodepth/test.pfm'
    loader = PFMLoader(color=False, compress=False)
    npData = loader.load_pfm(fPath)
    xDataLen = npData.shape[1]
    yDataLen = npData.shape[0]
    # dEData = pd.DataFrame(npData)
    dEData = npData.transpose()
    ...
    gPath = 'ground_monodepth/test.csv'
    dGtData = pd.read_csv(gPath)
    dGtData = dGtData * 1000.
    print(dGtData)
    print(dEData)    
    dError = dGtData.values - dEData
    # dError = np.abs(dError) 
    print(dError.shape)
    
    fig1 = plt.figure(figsize=(24, 12))
    ax = plt.axes(projection='3d')
    depth3D = ax.contour3D(x, y, dEData, 20, cmap=cm.jet, label='Estimation depth map')
    ...
    ax = plt.axes(projection='3d')
    depthEr = ax.contour3D(x, y, dGtData.values, 20, cmap=cm.jet, label='Ground Truth depth map')
    ax.set_xlabel(r"x (mm)"      ,  fontsize=20)
    ax.set_ylabel(r"y (mm)"      ,  fontsize=20)
    ...
    ax = plt.axes(projection='3d')
    depthEr = ax.contour3D(x, y, dError, 20, cmap=cm.jet, label='Ground Truth depth map')
    ax.set_xlabel(r"x (mm)"      ,  fontsize=20)
    ax.set_ylabel(r"y (mm)"      ,  fontsize=20)
